...
export default function TrustSection() {
    ...
    const skewX = useTransform(smoothVelocity, [-1000, 1000], [-35, 35]);
    const skewXReverse = useTransform(smoothVelocity, [-1000, 1000], [35, -35]);
            ...
            <div className="flex flex-col gap-8 py-20 pointer-events-none z-0">
                <motion.div style={{ x: xMove, skewX }} className="flex whitespace-nowrap gap-12">
                    {[...VALUES, ...VALUES, ...VALUES].map((val, i) => (
                        <span key={i} className="text-[10vw] font-serif font-bold text-void-light/5 text-starlight/20 leading-none">
                            {val}
                        </span>
                    ))}
                </motion.div>
                <motion.div style={{ x: xMoveReverse, skewX: skewXReverse }} className="flex whitespace-nowrap gap-12 -ml-[50%]">
                    {[...VALUES, ...VALUES, ...VALUES].reverse().map((val, i) => (
                        <span key={i} className="text-[10vw] font-serif font-bold text-gold-dim/40 leading-none">
                            {val}
                        </span>
                    ))}
                </motion.div>
                    ...
                    {[
                        ...
                        >
                            <div className="relative group">
                                <h3 className="text-[8rem] font-display text-transparent bg-clip-text bg-gradient-to-br from-gold-liquid to-gold-dim leading-none relative z-10 transition-transform duration-500 group-hover:scale-110 group-hover:-rotate-2">
                                    {stat.value}
                                </h3>
